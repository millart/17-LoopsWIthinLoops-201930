...
def multiply_numbers(sequence_of_lists):
    ...
    for k in range(len(sequence_of_lists)):
        one_list = sequence_of_lists[k]
        for j in range(len(one_list)):
            sequence_of_lists[k][j] = one_list[j] * (k + 1)
...
def run_test_sum_numbers():
    ...
    expected = 22
    answer = sum_numbers(([21, 5, -4], [0, 0, 5], [-12, 4, 3]))
    print('Expected and actual are:', expected, answer)
...
def print_characters(sequence_of_strings):
    ...
    for k in range(len(sequence_of_strings)):
        one_list = sequence_of_strings[k]
        for j in range(len(one_list)):
            print(one_list[j])
...
def print_characters_slanted(sequence_of_strings):
    ...
    for k in range(len(sequence_of_strings)):
        ...
        for j in range(len(one_list)):
            adding_space = ' ' * j
            print_this = adding_space + one_list[j]
            print(print_this)
